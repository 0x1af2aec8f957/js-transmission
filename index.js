...
export default function (option) {
  ...
  return option.header && Object.assign(option.header, this.header),
    ...
    new Promise((resolve, reject) => {
      const state_Change = () => { // ajax 状态码发生改变
          const {readyState, 
                 status,
                 statusText,
                 responseText,
              ...
              return xmlHttp.getResponseHeader('location') /* 支持重定向到另一个页面地址 */
                ? window.location.href = xmlHttp.getResponseHeader('location') : !(status - 200) ? /*! JSON/XML格式的数据才能被解析 */
                  success
                    ...
                    ? JSON.parse(
                      this.decode ? this.decode(responseText) : responseText)
                    : this.decode
                      ? this.decode(responseXML)
                      : responseXML, xmlHttp.getAllResponseHeaders())
